    ...
    <main>
      <div class="container mx-auto px-5">
        ...
        <div class="mb-3 flex gap-3 justify-center items-center flex-wrap">
          <RouterLink v-if="mainone.alphabets.length > 0" v-for="alphabet in mainone.alphabets" :key="alphabet" class="cursor-pointer hover:text-orange-500 duration-300 hover:scale-150" @click="mainone.searchbyletterfunc(alphabet)" :to="{ name: 'ByLetter', params: { alphabet: alphabet.name } }">{{ alphabet.name }}</RouterLink>
        </div>
        <!-- !!!another app!!! -->
        ...
        <!-- <div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5 py-5" v-if="mainone.allthesearchletter.length > 0">
